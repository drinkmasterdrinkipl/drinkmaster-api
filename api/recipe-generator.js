...
const RecipeGeneratorScreen = ({ route }: any) => {
  ...
  const generateRecipe = async (overrideName?: string) => {
    ...
    try {
      ...
      const recipe = await getDrinkRecipe(cocktailToGenerate, [], language, user?.uid);
      ...
      // Aktualizuj statystyki na backendzie
      if (user?.uid) {
        try {
          await fetch(`${API_URL}/api/user/stats/increment/${user.uid}`, {
            method: 'POST',
            headers: {
          ...
          console.error('Failed to update backend stats:', error);
        }
      }
                ...
                <>
                  ...
                  <View style={styles.infoBar}>
                    <View style={[styles.infoBox, globalStyles.stableCard, { backgroundColor: COLORS.background }]}>
                      ...
                      <Text style={[styles.infoLabel, { color: COLORS.textSecondary }]}>
                        {language === 'pl' ? 'METODA' : 'METHOD'}
                      </Text>
                      <Text style={[styles.infoValue, { color: COLORS.text }]} numberOfLines={2} adjustsFontSizeToFit>
                        {language === 'pl' ? translateMethod(generatedRecipe.method) : generatedRecipe.method}
                      </Text>
                    </View>
                    ...
                    <View style={[styles.infoBox, globalStyles.stableCard, { backgroundColor: COLORS.background }]}>
                      ...
                      <Text style={[styles.infoLabel, { color: COLORS.textSecondary }]}>
                        {language === 'pl' ? 'SZKŁO' : 'GLASS'}
                      </Text>
                      <Text style={[styles.infoValue, { color: COLORS.text }]} numberOfLines={2} adjustsFontSizeToFit>
                        {language === 'pl' ? translateGlass(generatedRecipe.glassType) : generatedRecipe.glassType}
                      </Text>
                    </View>
                  ...
                  <View style={styles.section}>
                    <View style={styles.sectionHeader}>
                      ...
                      <Text style={[styles.sectionTitle, { color: COLORS.text }]}>
                        {language === 'pl' ? 'PRZYGOTOWANIE' : 'INSTRUCTIONS'}
                      </Text>
                    </View>
                  ...
                  <View style={styles.section}>
                    <View style={styles.sectionHeader}>
                      ...
                      <Text style={[styles.sectionTitle, { color: COLORS.text }]}>
                        {language === 'pl' ? 'SERWOWANIE' : 'SERVING'}
                      </Text>
                    </View>
                        ...
                        <View style={styles.servingItem}>
                          ...
                          <Text style={[styles.servingText, { color: COLORS.text }]}>
                            {generatedRecipe.ice === 'brak' || generatedRecipe.ice === 'none' 
                              ? (language === 'pl' ? 'Brak lodu' : 'No ice')
                              : `${language === 'pl' ? 'Lód' : 'Ice'}: ${generatedRecipe.ice}`}
                          </Text>
                        </View>
                      )}
                  ...
                  <View style={styles.actionButtons}>
                    ...
                    >
                      ...
                      <Text style={[styles.actionButtonText, { color: COLORS.primary }]}>
                        {language === 'pl' ? 'Nowy przepis' : 'New recipe'}
                      </Text>
                    </TouchableOpacity>
                  </View>
                </>
